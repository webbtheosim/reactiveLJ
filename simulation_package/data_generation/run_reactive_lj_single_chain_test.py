"""Run a single-chain ReactiveLJ test in a large dilute box.

This script is intended as a visual/diagnostic sanity check for sticker
interactions. It simulates one KG chain in a fixed box, with sticker identities
assigned on that single chain and ReactiveLJ enabled.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run single-chain ReactiveLJ test.")
    parser.add_argument(
        "--epsilon",
        type=float,
        required=True,
...
def build_integrator(
    ...
    # Keep sticky-sticky baseline repulsion inside ReactiveLJ to avoid double counting.
    ...
    data.update(
        {
            ...
            "test_name": "single_chain_reactive_lj",
        }
    )
...
def main() -> None:
    ...
    print("Force_mode=ReactiveLJ", flush=True)
