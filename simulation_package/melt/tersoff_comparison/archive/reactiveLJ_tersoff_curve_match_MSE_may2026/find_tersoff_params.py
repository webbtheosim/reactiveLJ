...
SANITY_CURVE_FIGSIZE = (6.6, 2.2)
SANITY_THIRD_BEAD_DISTANCE_COUNT = 6
SANITY_THIRD_BEAD_DISTANCE_POWER = 4.0
...
def make_curve_shape_check_plot(
    path: str,
    r: np.ndarray,
    third_distances: np.ndarray,
    reactive_curves: np.ndarray,
    tersoff_curves: np.ndarray,
    sigma: float,
) -> None:
    fig, axes = uplt.subplots(
        nrows=1,
        ncols=2,
        figsize=SANITY_CURVE_FIGSIZE,
        dpi=300,
    )
    plot_r_min = 0.95 * sigma
    plot_r_max = 1.5 * sigma
    ...
    r_plot = r[plot_mask]
    reactive_plot = reactive_curves[:, plot_mask]
    tersoff_plot = tersoff_curves[:, plot_mask]
    scaled_distances = third_distances / sigma
    cmap = uplt.Colormap("plasma")
    colorbar_min = float(np.min(scaled_distances))
    colorbar_max = float(np.max(scaled_distances))
    colorbar_range = colorbar_max - colorbar_min

    all_plot_values = np.concatenate([reactive_plot.ravel(), tersoff_plot.ravel()])
    finite_values = all_plot_values[np.isfinite(all_plot_values)]
    if finite_values.size == 0:
        raise ValueError("No finite curve values available for sanity-check plot.")
    y_min = float(np.min(finite_values))
    y_upper = max(2.0, 0.15 * abs(y_min))
    y_lower = 1.08 * y_min if y_min < 0.0 else y_min - 1.0

    for axis, curves, title in zip(
        axes,
        (reactive_plot, tersoff_plot),
        ("ReactiveLJ", "L-O Tersoff"),
        strict=True,
    ):
        for idx, dist in enumerate(scaled_distances):
            if colorbar_range > 0.0:
...
def main() -> None:
    ...
    sanity_third_distances = compute_third_bead_distances(
        r_inner=third_bead_min_sigma * cfg.sigma,
        r_outer=third_bead_max_sigma * cfg.sigma,
        exponent=SANITY_THIRD_BEAD_DISTANCE_POWER,
        count=SANITY_THIRD_BEAD_DISTANCE_COUNT,
    )
    ...
    for epsilon in args.epsilons:
        ...
        sanity_reactive_curves = reactive_curve_set(
            r_grid,
            epsilon=epsilon,
            cfg=cfg,
            third_distances=sanity_third_distances,
        )
        sanity_tersoff_curves = np.asarray(
            [
                tersoff_surrogate_curve(
                    r=r_grid,
                    params=fit_params,
                    r_cut=fit_params["r_cut"],
                    rik_distance=float(third_distance),
                    surrogate_cos_theta=cfg.surrogate_cos_theta,
                )
                for third_distance in sanity_third_distances
            ],
            dtype=np.float64,
        )
        ...
        sanity_plot_path = os.path.join(
            plot_dir,
            f"curve_shape_check_eps_{epsilon:g}.svg",
        )
        make_curve_shape_check_plot(
            path=sanity_plot_path,
            r=r_grid,
            third_distances=sanity_third_distances,
            reactive_curves=sanity_reactive_curves,
            tersoff_curves=sanity_tersoff_curves,
            sigma=cfg.sigma,
        )
        ...
        print(
            ...
            f"shape_check_plot={sanity_plot_path}",
            flush=True,
        )
