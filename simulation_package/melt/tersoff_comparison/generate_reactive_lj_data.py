#!/usr/bin/env python3
"""Run one KG melt replicate using ReactiveLJ sticker interactions."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run one ReactiveLJ replicate with the same protocol used for the "
            "Tersoff comparison."
        )
    )
    parser.add_argument(
        ...
        type=float,
        required=True,
        help="ReactiveLJ attraction strength.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-root",
        default=os.path.join(os.path.dirname(__file__), "reactive_lj_outputs"),
        help="Root directory for ReactiveLJ comparison outputs.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Base random seed. Defaults to the Tersoff comparison seed.",
    )
    parser.add_argument(
        "--init-min-dist",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Equilibration steps after enabling ReactiveLJ interactions.",
    )
    parser.add_argument(
        "--production-steps",
        type=int,
...
def _augment_metadata(path: str) -> None:
    with open(path, "r", encoding="utf-8") as handle:
        metadata = json.load(handle)

    metadata["interaction_model"] = "reactive_lj"
...
def main() -> None:
    args = parse_args()
    if args.epsilon <= 0.0:
        raise ValueError("ReactiveLJ comparison runs require epsilon > 0.")
    ...
    _augment_metadata(metadata_path)
    ...
    print("Interaction_model=reactive_lj", flush=True)
