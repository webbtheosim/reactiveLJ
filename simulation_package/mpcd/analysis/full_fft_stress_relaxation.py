#!/usr/bin/env python3
"""Compute full-FFT network relaxation curves for MPCD ReactiveLJ runs.

This standalone script reads every ``virial_tensor_log.gsd`` under an input
root and computes G(t) from the polymer-network configurational virial stress
recorded by the MPCD production runs. The MPCD virial log intentionally omits
the solvent kinetic stress, so the output is the polymer-network relaxation,
not a total solution stress relaxation.

The estimator follows the melt ``full_fft_stress_relaxation.py`` workflow:
off-diagonal stress components are mean-subtracted, autocovariances are
computed on the native lag grid with an unbiased full FFT estimator,
replicates are aggregated by epsilon, and a log-log G(t) vs time-lag plot is
written with the same compact plotting style used for the melt stress plot.
"""
...
_CACHE_ROOT = Path("/tmp") / f"mpcd-network-relaxation-cache-{os.getuid()}"
...
def parse_args() -> argparse.Namespace:
    ...
    mpcd_dir = script_dir.parent
    parser = argparse.ArgumentParser(
        description=(
            "Compute full-FFT polymer-network relaxation from MPCD "
            "virial_tensor_log.gsd files and plot G(t) by epsilon."
        )
    )
    parser.add_argument(
        "--input-root",
        type=Path,
        default=mpcd_dir / "data_generation" / "outputs",
        help="Root directory containing eps_*/rep_*/virial_tensor_log.gsd files.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=script_dir / "results" / "full_fft_network_relaxation",
        help="Directory for per-epsilon CSVs, diagnostics, and the SVG plot.",
    )
    parser.add_argument(
        "--n-jobs",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Optional epsilon subset to process. Default: all discovered epsilons.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=PLOT_MIN_G,
        help="Stop plotting each epsilon curve at the first mean G below this value.",
    )
    parser.add_argument(
        "--plot-name",
        default="network_relaxation_gt_vs_time_lag_by_epsilon.svg",
        help="SVG plot filename written under --output-dir.",
    )
    parser.add_argument(
        "--csv-name",
        default="network_relaxation_gt.csv",
        help="Per-epsilon aggregate CSV filename.",
    )
    parser.add_argument(
        "--joblib-verbose",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Plot every replicate curve colored by epsilon instead of only the "
            "aggregate mean curve for each epsilon."
        ),
    )
...
def epsilon_is_selected(epsilon: float, selected: Iterable[float] | None) -> bool:
    if selected is None:
        return True
    return any(math.isclose(epsilon, value, rel_tol=0.0, abs_tol=1.0e-12) for value in selected)
...
def discover_runs(
    ...
    for root, dirs, files in os.walk(input_root, topdown=True):
        ...
        if not epsilon_is_selected(epsilon_f, selected_epsilons):
            continue
        runs.append(
            RunSpec(
    ...
    runs.sort(key=lambda item: (item.epsilon, item.replicate, str(item.virial_path)))
    if max_replicates_per_epsilon > 0:
        kept: List[RunSpec] = []
        counts: Dict[float, int] = defaultdict(int)
        for run in runs:
            if counts[run.epsilon] >= max_replicates_per_epsilon:
                continue
            kept.append(run)
            counts[run.epsilon] += 1
        runs = kept
    return runs
...
def checkpoint_deduplicated_indices(steps: np.ndarray, gsd_path: Path) -> np.ndarray:
    ...
    for physical_idx, raw_step in enumerate(np.asarray(steps, dtype=np.int64)):
        step = int(raw_step)
    ...
    if discarded_frames:
        log(
            ...
            f"(logical_frames={len(kept_indices)}/{len(steps)}, "
...
def load_virial_series(virial_path: Path) -> Tuple[np.ndarray, np.ndarray]:
    with gsd.hoomd.open(str(virial_path), "r") as traj:
        ...
        for frame_idx in range(n_frames):
            ...
            if parsed is None:
                continue
            virial[count, :] = parsed
            steps[count] = frame_step_or_index(frame, frame_idx)
            count += 1
    ...
    keep = checkpoint_deduplicated_indices(steps, virial_path)
    return virial[keep, :], steps[keep]
...
def compute_shear_stress_autocovariance_fft(
    ...
    max_lag = max(1, min(int(max_lag), n_samples - 1))
    ...
    fft_length = 2 * n_samples
    spectrum = np.fft.rfft(components, n=fft_length, axis=0)
    spectrum *= np.conjugate(spectrum)
    acf_components = np.fft.irfft(spectrum, n=fft_length, axis=0)[: max_lag + 1].real
    counts = np.arange(n_samples, n_samples - max_lag - 1, -1, dtype=np.float64)
    acf_components /= counts[:, None]
...
def compute_segmented_shear_stress_autocovariance_fft(
    ...
    xz = virial_arr[:, 2]
    yz = virial_arr[:, 4]
    components = np.empty((virial_arr.shape[0], 3), dtype=np.float64)
    components[:, 0] = xy
    components[:, 1] = xz
    components[:, 2] = yz
    components -= np.mean(components, axis=0, keepdims=True)
...
def compute_replicate(
    ...
    max_time = min(
        float(stress_max_runtime_fraction) * runtime,
        float(max_lag_time),
    )
...
def aggregate_one_epsilon(epsilon: float, results: List[ReplicateResult]) -> AggregateResult:
    if not results:
        raise RuntimeError(f"No replicate results to aggregate for eps={epsilon:g}")
    ...
    if not np.any(populated):
        raise RuntimeError(f"No populated time points after aggregation for eps={epsilon:g}")
...
def aggregate_replicates(results: List[ReplicateResult]) -> Dict[float, AggregateResult]:
    by_epsilon: Dict[float, List[ReplicateResult]] = defaultdict(list)
    for result in results:
        by_epsilon[result.epsilon].append(result)
    return {
        epsilon: aggregate_one_epsilon(epsilon, eps_results)
        for epsilon, eps_results in sorted(by_epsilon.items())
    }
...
def write_network_relaxation_by_epsilon_plot(
    path: Path,
    epsilon_values: List[float],
    g_time_by_eps: Dict[float, np.ndarray],
    g_mean_by_eps: Dict[float, np.ndarray],
    g_weight_by_eps: Dict[float, np.ndarray],
    tau_r0: float,
    min_plot_g: float,
    ...
    for eps in epsilon_values:
        ...
        if mean.ndim != 1 or mean.shape[0] != len(lag_time):
            continue
        if use_log_binning:
            lag_time, mean = log_bin_timeseries_for_plot(
                lag_time,
                mean,
                g_weight_by_eps.get(eps),
    ...
    for idx, (eps, lag_time, mean) in enumerate(series):
        color = cmap(idx)
        stop_mask = (~np.isfinite(mean)) | (mean < min_plot_g)
    ...
    ax.set_ylabel(r"$G(t)$", fontsize=10)
...
def write_all_replicate_network_relaxation_plot(
    path: Path,
    ...
    sorted_results = sorted(results, key=lambda item: (item.epsilon, item.replicate))
    if not sorted_results:
        return

    path.parent.mkdir(parents=True, exist_ok=True)
    cmap = plt.get_cmap("plasma", max(1, len(epsilon_values)))
    color_by_epsilon = {
        epsilon: cmap(idx)
    ...
    for result in sorted_results:
        ...
        color = color_by_epsilon.get(result.epsilon, "#2b2b2b")
        label = None
        if result.epsilon not in labeled_eps:
            label = f"eps={result.epsilon:g}"
            labeled_eps.add(result.epsilon)
...
def write_summary(path: Path, results: List[ReplicateResult], aggregated: Dict[float, AggregateResult]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "description": (
            "Full-FFT polymer-network relaxation from MPCD virial_tensor_log.gsd "
            "files. Solvent kinetic stress is not included."
        ),
        "replicate_count": len(results),
        "epsilon_values": [float(epsilon) for epsilon in sorted(aggregated)],
        "replicates": [
            {
            ...
            for result in sorted(results, key=lambda item: (item.epsilon, item.replicate))
        ],
        "aggregates": {
            f"{epsilon:g}": {
            ...
            for epsilon, result in sorted(aggregated.items())
        },
    }
...
def main() -> None:
    ...
    log(
        ...
        f"{len(set(run.epsilon for run in runs))} epsilon groups"
    )
    log(f"Computing full-FFT network relaxation with n_jobs={n_jobs}")
    ...
    if not results:
        raise RuntimeError("No usable network-relaxation results were produced.")

    log("Aggregating replicate curves by epsilon")
    aggregated = aggregate_replicates(results)
    args.output_dir.mkdir(parents=True, exist_ok=True)

    epsilon_values = sorted(aggregated)
    g_time_by_eps = {epsilon: result.time for epsilon, result in aggregated.items()}
    g_mean_by_eps = {epsilon: result.mean for epsilon, result in aggregated.items()}
    g_weight_by_eps = {epsilon: result.weight_sum for epsilon, result in aggregated.items()}
    x_limits = compute_shared_time_lag_xlim(g_time_by_eps, g_mean_by_eps, float(args.tau_r0))

    for epsilon, result in aggregated.items():
        eps_dir = args.output_dir / f"eps_{epsilon:g}"
        write_timeseries(eps_dir / args.csv_name, result.time, result.mean, result.stderr)
        write_count_diagnostics(eps_dir / "network_relaxation_counts.csv", result)
        log(
            f"Aggregated eps={epsilon:g}: "
    ...
    if args.plot_all_curves:
        write_all_replicate_network_relaxation_plot(
            plot_path,
            results,
            epsilon_values,
            float(args.tau_r0),
        ...
        write_network_relaxation_by_epsilon_plot(
            plot_path,
            epsilon_values,
            g_time_by_eps,
            g_mean_by_eps,
    ...
    log(f"Wrote full-FFT network relaxation outputs to {args.output_dir}")
