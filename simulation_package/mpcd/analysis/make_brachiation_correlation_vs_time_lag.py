#!/usr/bin/env python3
"""Plot MPCD open-sticker autocorrelation versus normalized time lag."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Plot the MPCD mean open-sticker autocorrelation versus time lag "
            "normalized by tau_R for each ReactiveLJ stickiness."
        )
    )
    parser.add_argument(
        "--input-root",
        ...
        help="Directory containing eps_*/open_sticker_correlation.csv outputs.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=script_dir / "results" / "brachiation_correlation_vs_time_lag.svg",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional explicit epsilon list to plot. Defaults to all epsilons found.",
    )
    parser.add_argument(
        "--tau-r",
        type=float,
...
def discover_epsilon_dirs(input_root: Path) -> list[tuple[float, Path]]:
    ...
    for candidate in input_root.glob("eps_*"):
        ...
        if epsilon is None:
            continue
        csv_path = candidate / "open_sticker_correlation.csv"
        if csv_path.is_file():
    ...
    for requested in requested_epsilons:
        ...
        if match is None:
            raise FileNotFoundError(
                f"Could not find open_sticker_correlation.csv for "
                f"epsilon={requested:g} in {discovered}"
            )
        selected.append(match)
    return selected


def load_open_sticker_correlation(csv_path: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    if not discovered:
        raise FileNotFoundError(
            f"No eps_*/open_sticker_correlation.csv files found under {args.input_root}"
        )

    selected = select_epsilon_dirs(discovered, requested_epsilons=args.epsilons)
    if not selected:
    ...
    for idx, (epsilon, eps_dir) in enumerate(selected):
        time_lag, mean_corr, stderr_corr = load_open_sticker_correlation(
            eps_dir / "open_sticker_correlation.csv"
        )
    ...
    ax.set_ylabel(r"$C_b(t)$", fontsize=DEFAULT_LABEL_FONTSIZE)
    ...
    print(
        f"Wrote open-sticker autocorrelation plot for epsilons "
